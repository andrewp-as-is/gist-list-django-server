"""
curl -s https://gist.github.com/<LOGIN>/<ID>/archive/<VERSION>.zip -o <ID>.zip
"""
...
class View(_View):
    def get(self,request):
        ...
        gist_list = list(model.objects.filter(owner_id=user.id).only('id','version','owner__id','owner__login').select_related('owner'))
        line_list = []
        ...
        for gist in gist_list:
            ...
            line_list+=['set -x; curl -s %s -o %s.zip;' % (url,gist.id)]
